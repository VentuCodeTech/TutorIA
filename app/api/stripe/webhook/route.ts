...
function buildPriceToPlan() {
  ...
  if (standardId) {
    PRICE_TO_PLAN[standardId] = 'standard'
  }
  if (studentId) {
    PRICE_TO_PLAN[studentId] = 'student'
  }
  if (advancedId) {
    PRICE_TO_PLAN[advancedId] = 'advanced_pro'
  }
}
...
async function handlePaymentFailed(invoice: Stripe.Invoice, supabase: ReturnType<typeof createAdminClient>) {
...
export async function POST(request: NextRequest) {
  ...
  switch (event.type) {
    ...
    case 'customer.subscription.deleted': {
      ...
      await handleSubscriptionChange(event, subscription, supabase)
      break
    }
