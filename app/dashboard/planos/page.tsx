...
// Map planId from useUserPlan to plan key used in this page
...
return (
  ...
  <div className="max-w-6xl mx-auto">
    ...
    <div>
    <p className="font-semibold text-green-800">Assinatura ativada com sucesso!</p>p>
    <p className="text-sm text-green-700">Bem-vindo ao plano {planLabels[successPlan || ''] || successPlan}. Aproveite todos os recursos!</p>p>
    </div>div>
    </div>div>
  )}
    {canceled && (
    <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-center gap-3">
    <span className="text-2xl">ℹ️</span>span>
    <p className="text-yellow-800">Checkout cancelado. Seu plano não foi alterado.</p>p>
    </div>div>
  )}
  
    {/* Header */}
  <div className="text-center mb-12">
  <h1 className="text-4xl font-bold text-gray-900 mb-4">💎 Planos e Preços</h1>h1>
  <p className="text-lg text-gray-600">Escolha o plano ideal para seus objetivos de estudo</p>p>
  </div>div>
  ...
  <h2 className="text-xl font-bold text-gray-900 mb-1">Gratuito</h2>h2>
  <div className="flex items-end gap-1 mb-6">
  <span className="text-4xl font-bold text-gray-900">R$ 0</span>span>
  <span className="text-gray-500 mb-1">/mês</span>span>
  </div>div>
  <ul className="space-y-3 flex-1">
    {['20 questões por dia','Acesso às matérias básicas','Dashboard básico','Comunidade de estudantes'].map(f => (
    <li key={f} className="flex items-center gap-2 text-sm text-gray-700">
    <span className="text-green-500">✅</span>span> {f}
    </li>li>
    ))}
    ...
    <span className="text-red-400">❌</span>span> {f}
    </li>li>
    ))}
  </ul>ul>
  <button className={`mt-6 w-full py-3 rounded-xl font-semibold transition-colors ${isCurrentPlan('gratuito') ? 'bg-green-100 border-2 border-green-500 text-green-700 cursor-default' : 'border border-gray-300 text-gray-600 hover:bg-gray-50'}`}>
    ...
    {/* Standard */}
  <div className="bg-white rounded-2xl border border-gray-200 p-6 flex flex-col">
  <h2 className="text-xl font-bold text-gray-900 mb-1">Standard</h2>h2>
  <div className="flex items-end gap-1 mb-6">
  <span className="text-4xl font-bold text-gray-900">R$ 19,90</span>span>
  <span className="text-gray-500 mb-1">/mês</span>span>
  </div>div>
  <ul className="space-y-3 flex-1">
    ...
    <button className="mt-6 w-full py-3 rounded-xl bg-green-100 border-2 border-green-500 text-green-700 font-semibold cursor-default">
    ✓ Plano Atual
    </button>button>
    ) : (
    <button
      ...
      >
      {loading === 'standard' ? (
        <><span className="animate-spin inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full"></span>span> Aguarde...</>>
        ) : 'Assinar Standard'}
    </button>button>
  )}
  </div>div>
  ...
  <h2 className="text-xl font-bold text-gray-900 mb-1">Student</h2>h2>
  <div className="flex items-end gap-1 mb-6">
  <span className="text-4xl font-bold text-gray-900">R$ 49,90</span>span>
  <span className="text-gray-500 mb-1">/mês</span>span>
  </div>div>
  <ul className="space-y-3 flex-1">
  ...
  <h2 className="text-xl font-bold text-gray-900 mb-1">Advanced Pro</h2>h2>
  <div className="flex items-end gap-1 mb-6">
  <span className="text-4xl font-bold text-gray-900">R$ 99,90</span>span>
  <span className="text-gray-500 mb-1">/mês</span>span>
  </div>div>
  <ul className="space-y-3 flex-1">
    {[
    ...
    'Badge exclusivo de Elite',
      ...
      <li key={f} className="flex items-center gap-2 text-sm text-gray-700">
      <span className="text-yellow-500">⭐</span>span> {f}
      </li>li>
      ))}
  </ul>ul>
  ...
  <h3 className="text-2xl font-bold mb-2">🎓 Garanta seu futuro hoje!</h3>h3>
  <p className="text-indigo-100 mb-4">Mais de 10.000 estudantes já passaram no vestibular com o TutorIA. Comece agora e transforme seus estudos.</p>p>
  <div className="flex flex-wrap justify-center gap-6 text-sm">
  <span>✅ Cancele quando quiser</span>span>
  <span>✅ Sem fidelidade</span>span>
  <span>✅ Garantia de 7 dias</span>span>
  <span>✅ Pagamento seguro via Stripe</span>span>
  </div>div>
  </div>div>
...
export default function PlanosPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="animate-spin w-8 h-8 border-4 border-indigo-500 border-t-transparent rounded-full"></div>div></div>div>}>
    <PlanosContent />
    </Suspense>Suspense>
    );
}</></></></div>
